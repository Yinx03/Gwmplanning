...
from navsim.common.dataclasses import Scene, Trajectory, NAVSIM_INTERVAL_LENGTH, TrajectorySampling
...
def plot_bev_with_agent(scene: Scene, agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    ...
    return fig, ax
def plot_bev_with_agent_generated_trj(scene: Scene, agent_trj) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent_trj: navsim agent predicted trj
    :return: figure and ax object of matplotlib
    """

    human_trajectory = scene.get_future_trajectory(num_trajectory_frames=8)
    agent_trajectory = Trajectory(agent_trj['future_trajectory'],TrajectorySampling(
                num_poses=len(agent_trj['future_trajectory']),
